...
BAR30_ReturnTypeDef_T BAR30_Reset(BAR30_Sensor_T *handle){
	uint8_t res;
	while( BAR30_Write(handle, BAR30_COMMAND_RESET))
		HAL_Delay(1);
	return BAR30_OK;
}
...
BAR30_ReturnTypeDef_T BAR30_Get_AllPromData(BAR30_Sensor_T *handle){
	...
	uint8_t RxBuff[2];
	BAR30_Read(handle, BAR30_COMAND_PROM_READ_1,  RxBuff);
	handle->PromData[0] = RxBuff[1] | (RxBuff[0]<<8);
	BAR30_Read(handle, BAR30_COMAND_PROM_READ_2,  RxBuff);
	handle->PromData[1] = RxBuff[1] | (RxBuff[0]<<8);
	BAR30_Read(handle, BAR30_COMAND_PROM_READ_3,  RxBuff);
	handle->PromData[2] = RxBuff[1] | (RxBuff[0]<<8);
	BAR30_Read(handle, BAR30_COMAND_PROM_READ_4,  RxBuff);
	handle->PromData[3] = RxBuff[1] | (RxBuff[0]<<8);
    BAR30_Read(handle, BAR30_COMAND_PROM_READ_5,  RxBuff);
	handle->PromData[4] = RxBuff[1] | (RxBuff[0]<<8);
	BAR30_Read(handle, BAR30_COMAND_PROM_READ_6,  RxBuff);
	handle->PromData[5] = RxBuff[1] | (RxBuff[0]<<8);
    BAR30_Read(handle, BAR30_COMAND_PROM_READ_7,  RxBuff);
    handle->PromData[6] = RxBuff[1] | (RxBuff[0]<<8);
